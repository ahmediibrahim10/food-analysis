...
export async function POST(request:Request){
 try{
  ...
  const rl=await rateLimitSafe(`food:${clientKey(request)}`,12,60_000);
  ...
  if(!rl.ok)return NextResponse.json({error:"Too many food scans. Please wait a moment."},{status:429,headers:{...jsonHeaders(rid),"Retry-After":"60"}});
  ...
  if(!requestSize(request,14_000_000))return NextResponse.json({error:"Image request is too large."},{status:413,headers:jsonHeaders(rid)});
  ...
  if(data.length>11_000_000)return NextResponse.json({error:"Image is too large. Please use a smaller photo."},{status:413});
  ...
  try{
    ...
    if(base){
      const r=await fetchWithTimeout(new URL("/api/nutrition/resolve",request.url),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:result.items})});
  ...
  logProviderEvent({requestId:rid,provider:"Gemini",operation:"food-vision",ok:true});
  return NextResponse.json(result,{headers:jsonHeaders(rid)});
 }catch(error){console.error("Gemini food scan failed",error instanceof Error?error.name:"unknown");return NextResponse.json({error:safeServerMessage(error,"Gemini analysis failed. Please try again." )},{status:500,headers:jsonHeaders(rid)});}
}
